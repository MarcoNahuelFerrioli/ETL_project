...
WHERE  dw_load_ts = (SELECT MAX(dw_load_ts) FROM fact_appointments);""")
...
SELECT * FROM appointments WHERE appointment_id > :last_id""")

params={"last_id": last_id}

with engine_mysql.connect() as mysql_conn:
    df_slots_raw = pd.read_sql(query_extract_data_slots, mysql_conn)
    df_status_raw = pd.read_sql(query_extract_data_status, mysql_conn)
    df_patients_raw = pd.read_sql(query_extract_data_patients, mysql_conn)
    df_appointment_raw = pd.read_sql(query_extract_data_appointment, mysql_conn, params=params)
